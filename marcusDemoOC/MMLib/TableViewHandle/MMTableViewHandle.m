...
- (MMResult)insertRow:(MMRow *)row atIndexPath:(NSIndexPath *)indexPath withRowAnimation:(UITableViewRowAnimation)animation
{
    ...
    if ((MMCheckArrayInRange(indexPath.section, _tableModel.sections) && MMCheckUsignNumberUnder(indexPath.row, [_tableModel.sections[indexPath.section] numberOfRows]+1)) || (indexPath.section == _tableModel.numberOfSections && indexPath.row == 0))
    {
...
- (NSInteger)numberOfRowsInSection:(NSInteger)index
{
    TableCheck(-1);
    ModelCheck(-1);
    return MMCheckArrayInRange(index, _tableModel.sections)?[self sectionWithIndex:index].numberOfRows:-1;
}
...
{
    ModelCheck();
    if (MMCheckArrayInRange(section, _tableModel.sections))
    {
