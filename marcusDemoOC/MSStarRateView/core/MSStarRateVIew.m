...
- (void)buildDataAndUI {
    ...
    _isAllowChange = YES;       //默认为YES
    if (!_foregroundImageName) {
        _foregroundImageName = @"star-on.png";
    }
    if (!_backgroundImageName) {
        _backgroundImageName = @"star-off.png";
    }
    
    self.foregroundStarView = [self createStarViewWithImage:_foregroundImageName];
    self.backgroundStarView = [self createStarViewWithImage:_backgroundImageName];
...
- (void)layoutSubviews {
    ...
    __weak MSStarRateView *weakSelf = self;
